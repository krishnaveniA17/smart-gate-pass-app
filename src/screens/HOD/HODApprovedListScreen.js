...
export default function HODApprovedListScreen() {
  ...
  const load = useCallback(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.warn("HOD approved list error:", err);
        setLoading(false);
        setRefreshing(false);
      }
    );
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.metaRow}>
                <View style={styles.metaChip}>
                  ...
                  <Text style={styles.metaText}>Mentor: {mentorName}</Text>
                </View>
                <View style={styles.metaChip}>
                  <Feather name="clock" size={12} color="#aaa" />
                  <Text style={styles.metaText}>{fmt(item.hodApproval?.at)}</Text>
                </View>
              </View>
            </View>

            {/* ✅ Badge (restored color) */}
    ...
    <View style={styles.safe}>
      ...
      {items.length === 0 ? (
        <View style={styles.center}>
          ...
          <Text style={styles.empty}>No approved requests yet.</Text>
        </View>
      ) : (
        <FlatList
          data={items}
...
const styles = StyleSheet.create({
  ...
  // ✅ Restored original bright green
  badgeGreen: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
